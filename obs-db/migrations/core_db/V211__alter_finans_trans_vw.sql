...
insert ignore into `b_eventaction_mapping`(`id`,`event_name`,`action_name`,`process`,`is_deleted`,`is_synchronous`) 
values (null,'Reconnection Service','Reconnect','workflow_events','N','N');

-- Disconnect Unpaid Customers Job --
SET @JOBID=(SELECT id FROM job WHERE name='DISCONNECT_UNPAID_CUSTOMERS');
DELETE FROM `job_parameters` WHERE job_id=@JOBID;

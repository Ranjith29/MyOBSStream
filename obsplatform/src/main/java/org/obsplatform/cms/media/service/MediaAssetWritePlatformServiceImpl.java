...
public class MediaAssetWritePlatformServiceImpl implements MediaAssetWritePlatformService {
	...
	public CommandProcessingResult createMediaAsset(final JsonCommand command) {
		...
		if("ADVANCEMEDIA".equalsIgnoreCase(mediaTypeCheck)){
			...
			if(command.hasParameter("mediaRating")){
				mediaAsset.setRating(command.bigDecimalValueOfParameterNamed("mediaRating"));	
			}
			if(command.hasParameter("duration")){
				mediaAsset.setDuration(command.stringValueOfParameterNamed("duration"));
			}
			if(command.hasParameter("mediatype")){
				mediaAsset.setType(command.stringValueOfParameterNamed("mediatype"));
			}
			if(command.hasParameter("mediaImage")){
				mediaAsset.setImage(command.stringValueOfParameterNamed("mediaImage"));
			}

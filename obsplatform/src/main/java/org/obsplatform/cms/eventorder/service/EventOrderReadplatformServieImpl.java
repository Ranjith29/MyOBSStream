...
public class EventOrderReadplatformServieImpl implements EventOrderReadplatformServie {
	...
	@Autowired
	public EventOrderReadplatformServieImpl(final PlatformSecurityContext context,final RoutingDataSource dataSource) {
	...
	/** this is new method for get price */
	@Override
	public BigDecimal retriveEventPriceNew(String fType, String oType,
			Long clientId, Long eventId) {
		final String sql = "select price from b_mod_pricing where format_type=? and Opt_type=? and client_typeid=(select category_type from m_client where m_client.id = ?) and is_deleted='n' and event_id= ? ";
		return jdbcTemplate.queryForObject(sql, new Object[]{fType,oType,clientId,eventId},BigDecimal.class);
	}
}
@SuppressWarnings("unused")

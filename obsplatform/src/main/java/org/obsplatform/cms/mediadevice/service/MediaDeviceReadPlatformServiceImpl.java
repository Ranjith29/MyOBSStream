...
public class MediaDeviceReadPlatformServiceImpl implements MediaDeviceReadPlatformService{
	...
	@Autowired
	public MediaDeviceReadPlatformServiceImpl (final PlatformSecurityContext context,final RoutingDataSource dataSource) {
	...
	private static final class MediaDeviceMapper implements RowMapper<MediaDeviceData> {
		public String mediaDeviceSchema() {
			 return "  SELECT a.id AS deviceId,a.client_id AS clientId,mc.code_value AS clientType,mc.id AS clientTypeId,b.balance_amount as balanceAmount,cc.currency " +
					"  FROM b_allocation a,m_code_value mc, m_client c left join b_client_balance b on b.client_id = c.id left join b_client_address ca on (c.id=ca.client_id and address_key='PRIMARY')"+
					" left join b_country_currency cc on (cc.country=ca.country ) WHERE  a.client_id = c.id AND " +
					"  mc.id = c.category_type AND serial_no = ? AND a.is_deleted = 'N' UNION SELECT a.id,a.client_id,mc.code_value,mc.id, b.balance_amount as balanceAmount,cc.currency" +
					"  FROM b_owned_hardware a, m_code_value mc, m_client c left join b_client_balance b on b.client_id = c.id left join b_client_address ca on (c.id=ca.client_id and address_key='PRIMARY')"+
					"left join b_country_currency cc on (cc.country=ca.country ) WHERE a.client_id = c.id AND mc.id = c.category_type" +

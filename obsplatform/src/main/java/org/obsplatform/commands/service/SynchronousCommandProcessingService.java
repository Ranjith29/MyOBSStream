...
public class SynchronousCommandProcessingService implements CommandProcessingService {
	...
	private NewCommandSourceHandler findCommandHandler(final CommandWrapper wrapper) {
		...
		if (wrapper.isConfigurationResource()) {
			...
			}else if (wrapper.isClientOrderNoRecord()) {
				handler = applicationContext.getBean("deleteCustomerNoRecordCommandHandler",NewCommandSourceHandler.class);
			}else if (wrapper.isUpdateClientRequiredFields()) {
				handler = applicationContext.getBean("updateClientRequiredFieldsCommandHandler",NewCommandSourceHandler.class);
			}else {
				throw new UnsupportedCommandException(wrapper.commandName());
			}
			// end of client
			...
			}else if (wrapper.isDeleteOrderNoRecord()) {
				handler = applicationContext.getBean("deleteOrderNoRecordCommandHandler",NewCommandSourceHandler.class);
			}
			else if (wrapper.isNewPasswordRequest()) {
				handler = applicationContext.getBean("newPasswordRequestCommandHandler",NewCommandSourceHandler.class);
		    }
			else if (wrapper.isResetPasswordRequest()) {
				handler = applicationContext.getBean("resetPasswordRequestCommandHandler",NewCommandSourceHandler.class);
		    }
			...
			else if (wrapper.isDeleteAddon()) {
				  handler = applicationContext.getBean("deleteOrderAddonsCommandHandler",NewCommandSourceHandler.class);
			    }else if (wrapper.isCancelAddOn()) {
					  handler = applicationContext.getBean("cancelOrderAddOnsCommandHandler",NewCommandSourceHandler.class);
			    }else  {
		    	 throw new UnsupportedCommandException(wrapper.commandName());
		   }
	       ...
	       if (wrapper.isCreate()) {
	        ...
	        }else if (wrapper.isRefundPayment()) {
	        	handler = applicationContext.getBean("refundPaymentCommandHandler", NewCommandSourceHandler.class);
	        }else {
	        	throw new UnsupportedCommandException(wrapper.commandName());
	        }
		             ...
		             if(wrapper.isCreateEventOrder()) {
			          ...
			          handler = applicationContext.getBean("closeEventPriceCommandHandler",NewCommandSourceHandler.class);
			           ...
			           throw new UnsupportedCommandException(wrapper.commandName());
						...
						} if(wrapper.isPaymentProspect()){
							handler = applicationContext.getBean("createPGProspectCommandHandler",NewCommandSourceHandler.class);
						}
				...
				if(wrapper.isCreate()){
					...
					handler = applicationContext.getBean("updateClientAdditionalInfoCommandHandler",NewCommandSourceHandler.class);
			    	   ...
			    	   if(wrapper.isCreate()){
			    		   handler = applicationContext.getBean("createSelfCareCommandHandler",NewCommandSourceHandler.class);
			        		...
			        		handler = applicationContext.getBean("updateSelfcarePasswordCommandHandler",NewCommandSourceHandler.class);	
					   } else{
				           throw new UnsupportedCommandException(wrapper.commandName());
				       }
        		 ...
        		 if(wrapper.isCreatePromotionCode()) {
				   ...
				   if(wrapper.isCreate()) {
			        	 ...
			        	 handler = applicationContext.getBean("cancelCreditDistributionCommandHandler", NewCommandSourceHandler.class);
			        }else {
				           throw new UnsupportedCommandException(wrapper.commandName());
	      ...
	      } else if (wrapper.isRecurringResource()) {
				if (wrapper.isAuthorize()) {
					handler = applicationContext.getBean("authorizeRecurringBillingCommandHandler", NewCommandSourceHandler.class);
				} else if (wrapper.isUpdateProfile()) {
					handler = applicationContext.getBean("updateRecurringBillingOrderIdCommandHandler", NewCommandSourceHandler.class);
				} else if (wrapper.isCreate()) {
					handler = applicationContext.getBean("createRecurringBillingProfileCommandHandler", NewCommandSourceHandler.class);
				} else {
					 throw new UnsupportedCommandException(wrapper.commandName());
				  }
			
			}else if (wrapper.isDepartmentResource()) {
				if (wrapper.isCreateDepartment()) {
					handler = applicationContext.getBean("createDepartmentCommandHandler", NewCommandSourceHandler.class);
				}else if(wrapper.isUpdateDepartment()){
					handler = applicationContext.getBean("updateDepartmentCommandHandler", NewCommandSourceHandler.class);
				}else if(wrapper.isDeleteDepartment()){
					handler = applicationContext.getBean("deleteDepartmentCommandHandler", NewCommandSourceHandler.class);
				}else {
				   throw new UnsupportedCommandException(wrapper.commandName());
				} 
			}else if (wrapper.isWifiResource()) {
				if (wrapper.isCreateWifi()) {
					handler = applicationContext.getBean("createWifiCommandHandler", NewCommandSourceHandler.class);
				}else if(wrapper.isUpdateWifi()){
					handler = applicationContext.getBean("updateWifiByOrderIdCommandHandler", NewCommandSourceHandler.class);
				}else if(wrapper.isDeleteWifi()){
					handler = applicationContext.getBean("deleteWifiCommandHandler", NewCommandSourceHandler.class);
				}else {
				   throw new UnsupportedCommandException(wrapper.commandName());
				} 
			}else if(wrapper.isEmployeeResource()){
				  if(wrapper.isCreateEmployee()){
						 handler = applicationContext.getBean("createEmployeeCommandHandler",NewCommandSourceHandler.class);
					  }else if(wrapper.isUpdateEmployee()){
							 handler = applicationContext.getBean("updateEmployeeCommandHandler",NewCommandSourceHandler.class);
					  }else if(wrapper.isDeleteEmployee()){
							 handler = applicationContext.getBean("deleteEmployeeCommandHandler",NewCommandSourceHandler.class);
					  }else {
						 throw new UnsupportedCommandException(wrapper.commandName());
					  }
			 }else if (wrapper.iscreditcardPayment()) {
	             handler = this.applicationContext.getBean("creditcardPaymentCommandHandler", NewCommandSourceHandler.class);
		     }else if (wrapper.iscreditcardPaymentWorldpay()) {
	             handler = this.applicationContext.getBean("createOrderWorldpayCommandHandler", NewCommandSourceHandler.class);
		     }else if(wrapper.isExDirectoryResource()){
				  if(wrapper.isCreateIsExDirectory()){
						 handler = applicationContext.getBean("createIsExDirectoryCommandHandler",NewCommandSourceHandler.class);
					  }else if(wrapper.isUpdateIsExDirectory()){
							 handler = applicationContext.getBean("updateIsExDirectoryCommandHandler",NewCommandSourceHandler.class);
					  }else {
						 throw new UnsupportedCommandException(wrapper.commandName());
					  }
			 }else if(wrapper.isRegistrationFee()){
		    	  if(wrapper.isCreate()){
		    		  handler = applicationContext.getBean("createRegistrationFeeCommandHandler",NewCommandSourceHandler.class);
		    	  }else {
		    		  throw new UnsupportedCommandException(wrapper.commandName());
		    	  }
		      }else if (wrapper.isServicePartnerMappingResource()) {
	               if(wrapper.isCreateServicePartnerMapping()){
	                   handler = applicationContext.getBean("createServicePartnerMappingCommandHandler",NewCommandSourceHandler.class);
	                  }else if(wrapper.isUpdateServicePartnerMapping()){
	                	  handler = applicationContext.getBean("updateServicePartnerMappingCommandHandler",NewCommandSourceHandler.class);
	                  }else if(wrapper.isDeleteServicePartnerMapping()){
	                	  handler = applicationContext.getBean("deleteServicePartnerMappingCommandHandler",NewCommandSourceHandler.class);
	                  }
		      }else if(wrapper.isTicketAssignRuleResource()){
		    	  if(wrapper.isCreateTicketAssignRule()){
		    		  handler = applicationContext.getBean("createTicketAssignRuleCommandHandler",NewCommandSourceHandler.class);
		    	  }else if(wrapper.isUpdateTicketAssignRule()){
		    		  handler = applicationContext.getBean("updateTicketAssignRuleCommandHandler",NewCommandSourceHandler.class);
		    	  }else if(wrapper.isDeleteTicketAssignRule()){
		    		  handler = applicationContext.getBean("deleteTicketAssignRuleCommandHandler",NewCommandSourceHandler.class);
		    	  }
		      }else if(wrapper.isSetupFee()){
		    	  if(wrapper.isCreate()){
		    		  handler = applicationContext.getBean("createSetupFeeCommandHandler",NewCommandSourceHandler.class);
		    	  }else {
		    		  throw new UnsupportedCommandException(wrapper.commandName());
		    	  }
		      }else {

...
public class PaymentsApiResource {
	...
	public String retrieveDetailsForPayments(@QueryParam("clientId") final Long clientId,@Context final UriInfo uriInfo) {
		...
		final String referencenum = this.readPlatformService.retreieveNum(clientId);
		final PaymentData paymentData=new PaymentData(data, depositDatas,referencenum);
	...
	@Consumes({ MediaType.APPLICATION_JSON })
    @Produces({MediaType.APPLICATION_JSON})
	public String crediCardProcessing(final String apiRequestBodyAsJson) {
		
		final CommandWrapper commandRequest = new CommandWrapperBuilder().creditcardPayment().withJson(apiRequestBodyAsJson).build();
		final CommandProcessingResult result =  this.commandsSourceWritePlatformService.logCommandSource(commandRequest);
		return this.toApiJsonSerializer.serialize(result);
				
		//String response = this.paymentGatewayWritePlatformService.crediCardProcess(apiRequestBodyAsJson);
		//return OnlinePaymentMethod(response);	
	}
	
	/**
	 * This method is using for refund payment with payment id
	 */
	 @PUT
	 @Path("refundpayment/{paymentId}")
	 @Consumes({ MediaType.APPLICATION_JSON })
	 @Produces({ MediaType.APPLICATION_JSON })
	 public String refundPayment(@PathParam("paymentId") final Long paymentId,final String apiRequestBodyAsJson) {
		 
			final CommandWrapper commandRequest = new CommandWrapperBuilder().refundPayment(paymentId).withJson(apiRequestBodyAsJson).build();
			final CommandProcessingResult result = this.writePlatformService.logCommandSource(commandRequest);
			return this.toApiJsonSerializer.serialize(result);
		}
}

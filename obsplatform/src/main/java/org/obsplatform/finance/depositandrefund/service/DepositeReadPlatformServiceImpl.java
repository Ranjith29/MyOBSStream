...
public class DepositeReadPlatformServiceImpl implements DepositeReadPlatformService {
	...
	private static final class DepositeMapper implements RowMapper<FeeMasterData> {
		...
		public String schema() {
			return "fm.id AS id,fm.fee_code AS feeCode,fm.is_refundable as isRefundable,fm.charge_code AS chargeCode,"+
				      " ifnull(round(fd.amount, 2), fm.default_fee_amount) AS amount FROM b_fee_master fm LEFT JOIN b_client_address ca ON ca.client_id = ? "+
				       " LEFT JOIN b_state s ON s.state_name = ca.state  LEFT JOIN m_client mc ON mc.id = ca.client_id LEFT JOIN b_priceregion_detail pd ON ( pd.state_id = ifnull((SELECT DISTINCT c.id FROM b_fee_detail a,"+
				       ...
				       " LEFT JOIN b_priceregion_master prm ON prm.id = pd.priceregion_id LEFT JOIN b_fee_detail fd ON (fd.fee_id = fm.id AND fd.region_id = prm.id AND fd.category_id = mc.category_type AND fd.is_deleted = 'N') "+
		...
		public FeeMasterData mapRow(final ResultSet rs, final int rowNum)
			...
			return new FeeMasterData(id, feeCode, " ", "Deposit", chargeCode, defaultFeeAmount, isRefundable, null);

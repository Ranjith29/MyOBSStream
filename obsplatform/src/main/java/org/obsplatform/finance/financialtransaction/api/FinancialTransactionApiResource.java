...
public class FinancialTransactionApiResource {
	...
	public FinancialTransactionApiResource(final BillMasterReadPlatformService billMasterReadPlatformService,
			final PlatformSecurityContext context,final DefaultToApiJsonSerializer<FinancialTransactionsData> toApiJsonSerializer,
	...
	public Response retrieveFinancialTransactionalData(@PathParam("clientId") final Long clientId,@Context final UriInfo uriInfo,
			@QueryParam("downloadType") final String downloadType,@QueryParam("fromDate") final Long start,
			@QueryParam("toDate") final Long end) throws IOException {
		...
		if (downloadType.equalsIgnoreCase("csv")) {
			...
			try {
				...
				table.addCell("TransactionCategory");
				table.addCell("DebitAmount");
				table.addCell("CreditAmount");

				for (FinancialTransactionsData data : financialTransactionsData) {
					table.addCell(data.getUserName());
					table.addCell(data.getTransactionId() + "");
					table.addCell(data.getTransDate() + "");
					table.addCell(data.getTransactionType());
					table.addCell(data.getTransactionCategory());
					table.addCell(data.getAmount() + "");
					table.addCell(data.getCreditAmount() + "");
				}
				table.completeRow();
				document.add(table);
				document.close();

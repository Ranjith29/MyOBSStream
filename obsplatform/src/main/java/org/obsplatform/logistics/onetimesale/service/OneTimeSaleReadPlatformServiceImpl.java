...
public class OneTimeSaleReadPlatformServiceImpl implements	OneTimeSaleReadPlatformService {
	...
	private static final class SalesDataMapper implements RowMapper<OneTimeSaleData> {
		...
		public String schema() {
			...
			return "o.id AS id,i.item_code AS itemCode, i.item_class as itemClass, a.serial_no as serialNo,o.sale_date as saleDate,o.charge_code AS chargeCode," +
					...
					+ " id.id as itemDetailId,id.provisioning_serialno as provserialnumber,id.quality as quality,id.remarks as remarks FROM b_item_master i JOIN b_onetime_sale o ON i.id=o.item_id and o.is_deleted='N' " +
		...
		public OneTimeSaleData mapRow(ResultSet rs, int rowNum)throws SQLException {
			...
			return new OneTimeSaleData(id, saleDate, itemCode, chargeCode,quantity, totalPrice,haardwareAllocated,itemClass,serialNo,units,saleType,warrantyDate,propertyCode,itemDetailId,provserialnumber,quality, remarks);
	...
	private static final class AllocationDataMapper implements	RowMapper<AllocationDetailsData> {
		...
		public String schema() {
			return "  a.id AS id,id.id AS itemDetailId,id.quality as quality,a.status as hardwareStatus,i.item_description AS itemDescription, " +
					" a.serial_no AS serialNo,a.allocation_date AS allocationDate, id.warranty_date As warrantyDate" +
		...
		public AllocationDetailsData mapRow(ResultSet rs, int rowNum) throws SQLException {
			...
			return new AllocationDetailsData(id,itemDescription, serialNo, allocationDate, itemDetailId, null, quality, hardwareStatus, warrantyDate);
	...
	/* (non-Javadoc)
	 * @see #retrieveCustomerExistsSaleDevices(java.lang.Long, java.lang.Long)
	 */
	@Override
	public Long retrieveCustomerExistsSaleDevices(final Long clientId, final Long itemId) {
		String sql=null;
		if(itemId !=null){
			
			sql= " SELECT ifnull(count(bos.id),0) FROM b_onetime_sale bos,b_item_master im WHERE bos.item_id = im.id AND bos.is_deleted='N' " +
					" AND bos.device_mode = 'NEWSALE' AND bos.hardware_allocated='ALLOCATED' AND bos.client_id = ?  AND im.id ="+itemId;
		
		}else{
			sql=" SELECT ifnull(count(ba.id), 0) FROM b_onetime_sale bos, b_allocation ba WHERE bos.id = ba.order_id AND bos.item_id = ba.item_master_id"+
			        " AND bos.is_deleted = 'N'and ba.is_deleted='N' AND bos.device_mode = 'NEWSALE' AND bos.hardware_allocated = 'ALLOCATED' AND ba.client_id = ? ";
					
		}
		return this.jdbcTemplate.queryForLong(sql, new Object [] {clientId});
	}

	@Override
	public List<ItemData> retrieveItemByCategoryType(Long itemId) {
			 this.context.authenticatedUser();
			 final ItemMapperType mapper = new ItemMapperType();
			 final String sql = "select " + mapper.schema() +" AND i.item_category_id=(select item_category_id from b_item_master where id= ? )";

			return this.jdbcTemplate.query(sql, mapper, new Object[] { itemId });
		
	}//end 
	
	private static final class ItemMapperType implements RowMapper<ItemData> {

		public String schema() {
			
			return " i.id AS id,i.item_code AS itemCode,i.units AS units,i.unit_price AS unitPrice, " +
					" i.item_category_id as itemCategoryId, mcv.code_value as itemCategoryName " +
					" FROM b_item_master i  " +
					"join  m_code_value mcv ON mcv.id=i.item_category_id " +
		...
		public ItemData mapRow(ResultSet rs, int rowNum) throws SQLException {
			...
			final Long itemCategoryId=rs.getLong("itemCategoryId");
			final String itemCategoryName = rs.getString("itemCode");

			return new ItemData(id, itemCode, units, units, units, null, 0,unitPrice,null,null,null,null,null,itemCategoryId,itemCategoryName);
		}
	}

	@Override
	public ItemData retrieveItemByCategoryTypeByItemId(Long itemId) {
		this.context.authenticatedUser();
		 final ItemMapperType mapper1 = new ItemMapperType();
		 final String sql = "select " + mapper1.schema() +" AND i.id= ?";
		return this.jdbcTemplate.queryForObject(sql, mapper1, new Object[] { itemId });
	}

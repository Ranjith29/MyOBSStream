  ...
  public String retriveSingleItemDetail(@Context final UriInfo uriInfo, @PathParam("itemId") final Long itemId) {
   ...
   return this.toApiJsonSerializerForItem.serialize(settings,clientDatafinal,RESPONSE_ITEM_DETAILS_PARAMETERS);
	...
	public String retriveItemDetails(@Context final UriInfo uriInfo,@QueryParam("sqlSearch") final String sqlSearch, 
			         ...
			         @QueryParam("allocateid") final String allocateid, @QueryParam("quality") final String quality) {
		
		context.authenticatedUser().validateHasReadPermission(resourceNameForPermissions);
		final SearchSqlQuery searchItemDetails =SearchSqlQuery.forSearch(sqlSearch, offset,limit );
		final Page<ItemDetailsData> clientDatafinal = this.itemDetailsReadPlatformService.retriveAllItemDetails(searchItemDetails,officeName,itemCode,quality);
	...
	@Consumes({ MediaType.APPLICATION_JSON })
	@Produces({ MediaType.APPLICATION_JSON })
	public String deAllocateHardware(@PathParam("allocationId") final Long allocationId,final String apiRequestBodyAsJson) {
		
		 final CommandWrapper commandRequest = new CommandWrapperBuilder().deAllocateHardware(allocationId).withJson(apiRequestBodyAsJson).build();
		 final CommandProcessingResult result = this.commandsSourceWritePlatformService.logCommandSource(commandRequest);
		  return this.toApiJsonSerializerForItem.serialize(result);
	}
	...
	public String retriveItemDetailsBySerialNum(@QueryParam("query") final String query,@QueryParam("clientId") final Long clientId,@Context final UriInfo uriInfo){
		...
		final BigDecimal itemPrice = this.oneTimeSaleWritePlatformService.calculateChargeVariantPriceForItems(itemMasterData.getUnitPrice(), clientId, itemMasterData);
		itemMasterData.setUnitPrice(itemPrice);
		final ApiRequestJsonSerializationSettings settings = apiRequestParameterHelper.process(uriInfo.getQueryParameters());
	...
	public String retriveDropdownValues(@Context final UriInfo uriInfo){
		...
		final Collection<MCodeData> qualityDatas=this.mCodeReadPlatformService.getCodeValue(CodeNameConstants.CODE_ITEM_QUALITY);
		ItemDetailsData itemDetailsData=new ItemDetailsData(officeData,itemMasterData,qualityDatas);
		final ApiRequestJsonSerializationSettings settings = apiRequestParameterHelper.process(uriInfo.getQueryParameters());
		return this.toApiJsonSerializerForItem.serialize(settings,itemDetailsData,RESPONSE_DATA_GRN_IDS_PARAMETERS);
	}

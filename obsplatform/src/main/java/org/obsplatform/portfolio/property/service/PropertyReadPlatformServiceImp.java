...
/*			final String sql = "select "+ mapper.schema()+ "  and pm.property_code_type like '%"+propertyType+"%' AND pm.code like '%"+code+"%'  order by pm.id LIMIT 20";*/
	...
	@Override
	public List<String> retrieveclientProperties(final Long clientId) {
	...
	@Override
	public List<PropertyDeviceMappingData> retrievePropertyDeviceMappingData(final String serialNumber) {
		
		try {	
				context.authenticatedUser();
				final PropertyDeviceMapper mapper = new PropertyDeviceMapper();
				
				final String sql = "select pd.id as id,pd.client_id as clientId,pd.serial_number as serialnumber,pd.property_code as propertyCode " +

...
public class ActivationProcessCommandFromApiJsonDeserializer {
    ...
    public void validateForCreate(final String json, boolean isdeviceReq) {
      ...
      //  baseDataValidator.reset().parameter("fullname").value(fullname).notBlank().notExceedingLengthOf(50);
        ...
        /*if (fromApiJsonHelper.parameterExists("itemCategoryTypeId", element)) {
        	final Long itemCategoryTypeId = fromApiJsonHelper.extractLongNamed("itemCategoryId", element);
        	 baseDataValidator.reset().parameter("itemCategoryTypeId").value(itemCategoryTypeId).notNull().notExceedingLengthOf(45);
        }
        
        if (fromApiJsonHelper.parameterExists("itemCategoryName", element)) {
        	final String itemCategoryName = fromApiJsonHelper.extractStringNamed("itemCategoryName", element);
        	 baseDataValidator.reset().parameter("itemCategoryName").value(itemCategoryName).notExceedingLengthOf(50);
        }*/

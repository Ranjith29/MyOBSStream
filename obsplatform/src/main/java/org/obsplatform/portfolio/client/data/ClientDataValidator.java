...
public final class ClientDataValidator {
   ...
   //     final Boolean active = fromApiJsonHelper.extractBooleanNamed(ClientApiConstants.activeParamName, element);
       ...
       /* if (fromApiJsonHelper.parameterExists(ClientApiConstants.addressNoParamName, element)) {
        ...
        if (fromApiJsonHelper.parameterExists(ClientApiConstants.distributionBoxParamName, element)) {
            final String distributionBox = fromApiJsonHelper.extractStringNamed(ClientApiConstants.distributionBoxParamName, element);
            baseDataValidator.reset().parameter(ClientApiConstants.distributionBoxParamName).value(distributionBox).notNull()
                    .notExceedingLengthOf(100);
        }
        ...
        if (fromApiJsonHelper.parameterExists(ClientApiConstants.phoneParamName, element)) {
            ...
            baseDataValidator.reset().parameter(ClientApiConstants.homePhoneNumberParamName).value(homePhoneNumber).ignoreIfNull()
        ...
        if (fromApiJsonHelper.parameterExists("cardNumber", element)){
        	final String cardNumber = fromApiJsonHelper.extractStringNamed(ClientApiConstants.cardNumberParamName, element);
        	baseDataValidator.reset().parameter("cardNumber").value(cardNumber).notBlank().notExceedingLengthOf(20);
        	
        }
        
        if (fromApiJsonHelper.parameterExists("cardType", element)){
        	final String cardType = fromApiJsonHelper.extractStringNamed(ClientApiConstants.cardTypeParamName, element);
        	baseDataValidator.reset().parameter("cardType").value(cardType).notBlank().notExceedingLengthOf(20);
        	
        }
        
        if (fromApiJsonHelper.parameterExists("cvvNumber", element)){
        	final String cvvNumber = fromApiJsonHelper.extractStringNamed(ClientApiConstants.cvvNumberParamName, element);
        	baseDataValidator.reset().parameter("cvvNumber").value(cvvNumber).notBlank().notExceedingLengthOf(20);
        	
        }
        
        if (fromApiJsonHelper.parameterExists("expiryDate", element)){
        	final String expiryDate = fromApiJsonHelper.extractStringNamed(ClientApiConstants.expiryDateParamName, element);
        	baseDataValidator.reset().parameter("expiryDate").value(expiryDate).notBlank().notExceedingLengthOf(100);
        	
        }
        
        if (fromApiJsonHelper.parameterExists("type", element)){
        	final String type = fromApiJsonHelper.extractStringNamed(ClientApiConstants.typeParamName, element);
        	baseDataValidator.reset().parameter("type").value(type).notBlank().notExceedingLengthOf(20);

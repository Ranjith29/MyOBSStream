...
public class ClientProspectCommandFromApiJsonDeserializer {
	...
	private final Set<String> supportedParameters = new HashSet<String>(Arrays.asList("id", "sourceOfPublicityInt", "preferredPlanInt",
					...
					"statusRemark", "callStatus", "assignedTo", "notes", "isDeleted", "zipCode", "billAlign",
					"planCode", "contractPeriod", "paytermCode", "noOfConnections","dateFormat","totalPrice",
					"cardNumber","cardType","cvvNumber","cardExpiryDate","name","type","isPrepaid"));
	...
	public void validateForCreate(final String json) {
		...
		baseDataValidator.reset().parameter("name").value(name).notExceedingLengthOf(50);
		
		final String cardExpiryDate = fromApiJsonHelper.extractStringNamed("cardExpiryDate", element);
		baseDataValidator.reset().parameter("cardExpiryDate").value(cardExpiryDate).notExceedingLengthOf(50);
		
		final String cardNumber = fromApiJsonHelper.extractStringNamed("cardNumber", element);
		baseDataValidator.reset().parameter("cardNumber").value(cardNumber).notExceedingLengthOf(50);
		
		final String cardType = fromApiJsonHelper.extractStringNamed("cardType", element);
		baseDataValidator.reset().parameter("cardType").value(cardType).notExceedingLengthOf(50);
		
		final String cvvNumber = fromApiJsonHelper.extractStringNamed("cvvNumber", element);
		baseDataValidator.reset().parameter("cvvNumber").value(cvvNumber).notExceedingLengthOf(20);
		
		final String type = fromApiJsonHelper.extractStringNamed("type", element);
		baseDataValidator.reset().parameter("type").value(type).notExceedingLengthOf(20);
		
		checkOrderValidation(baseDataValidator, element);
	...
	private void checkOrderValidation(DataValidatorBuilder baseDataValidator, JsonElement element){
		
		final String planCode = fromApiJsonHelper.extractStringNamed("planCode", element);
		if(fromApiJsonHelper.parameterExists("planCode", element) && null != planCode){
			baseDataValidator.reset().parameter("planCode").value(planCode).notBlank().notExceedingLengthOf(50);
			
			final String contractPeriod = fromApiJsonHelper.extractStringNamed("contractPeriod", element);
			baseDataValidator.reset().parameter("contractPeriod").value(contractPeriod).notBlank().notExceedingLengthOf(50);
			
			final String noOfConnections = fromApiJsonHelper.extractStringNamed("noOfConnections", element);
			baseDataValidator.reset().parameter("noOfConnections").value(noOfConnections).notBlank().integerGreaterThanZero();
		}
	}
}

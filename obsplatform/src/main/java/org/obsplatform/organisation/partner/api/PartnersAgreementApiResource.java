...
import org.obsplatform.organisation.partner.service.PartnersAgreementReadPlatformService;
...
/**
 * @author Ranjith
 * this api class use to create ,update and delete agreements of partners
 *
 */
@Path("/agreements")
@Component
...
public class PartnersAgreementApiResource {
	 ...
	 * {@link AgreementData}.
	 */
	private final Set<String> RESPONSE_DATA_PARAMETERS = new HashSet<String>(Arrays.asList("id,agreementStatus,startDate,endDate,shareType,shareAmount"));
	...
	@GET
	@Path("offices/{partnerId}")//officeId
	@Consumes({ MediaType.APPLICATION_JSON })
	@Produces({ MediaType.APPLICATION_JSON })
	public String retrievePartnerOfficesData(	@PathParam("partnerId") final Long partnerId,@Context final UriInfo uriInfo) {

		context.authenticatedUser().validateHasReadPermission(resorceNameForPermission);
	    List<AgreementData> agreementData = this.agreementReadPlatformService.retrievePartnerOfficeData(partnerId);
		final ApiRequestJsonSerializationSettings settings = apiRequestParameterHelper.process(uriInfo.getQueryParameters());
		return this.toApiJsonSerializer.serialize(settings, agreementData,RESPONSE_DATA_PARAMETERS);
	}

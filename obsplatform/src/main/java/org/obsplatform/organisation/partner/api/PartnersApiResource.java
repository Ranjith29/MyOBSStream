...
/**
 * @author Ranjith
 * this api class use to create,update partners 
 */
@Path("/partners")
@Component
@Scope("singleton")
  ...
  @Autowired	
   public PartnersApiResource(final PlatformSecurityContext context,final ToApiJsonSerializer<PartnersData> toApiJsonSerializer,
	...
	private PartnersData handlePartnersTemplateData() {
		...
		final Collection<OfficeData> allowedParents = this.officereadPlatformService.retrieveAllParentOffices();
		final Collection<CodeValueData> partnerTypes=this.codeValueReadPlatformService.retrieveCodeValuesByCode(PARTNER_TYPE);
		
		return new PartnersData(countryData,statesData,citiesData,officeTypes,currencyData,allowedParents,configCurrency, partnerTypes);
	}
    ...
    public String retrievePartner(@PathParam("partnerId") final Long partnerId,@Context final UriInfo uriInfo) {
        ...
        if(settings.isTemplate()){
        	...
        	partner.setPartnerTypes(this.codeValueReadPlatformService.retrieveCodeValuesByCode(PARTNER_TYPE));
        }
	...
	@GET
    @Path("partnerTypeName/{partnerId}")
    @Consumes({ MediaType.APPLICATION_JSON })
    @Produces({ MediaType.APPLICATION_JSON })
    public String retrievePartnerTypeName(@PathParam("partnerId") final Long partnerId,@Context final UriInfo uriInfo) {

        context.authenticatedUser().validateHasReadPermission(resorceNameForPermission);
        final PartnersData partner = this.readPlatformService.retrieveSinglePartnerTypeName(partnerId);
        final ApiRequestJsonSerializationSettings settings = apiRequestParameterHelper.process(uriInfo.getQueryParameters());
        return this.toApiJsonSerializer.serialize(settings, partner, RESPONSE_DATA_PARAMETERS);
    }

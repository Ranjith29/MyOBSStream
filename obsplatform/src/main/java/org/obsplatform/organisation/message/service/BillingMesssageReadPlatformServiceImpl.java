...
public class BillingMesssageReadPlatformServiceImpl implements
	...
	private static EventActionReadPlatformService eventActionReadPlatformService;
	...
	private static final class BillingMessageDataForProcessingMapper implements
		...
		public String schema() {
			...
			return " md.id as id,md.message_to as messageto,md.message_from as messagefrom,md.subject as subject,md.header as header," +
					...
					" from b_message_data md  where DATE_FORMAT(md.created_date,'%y %m') =  DATE_FORMAT(CURDATE(),'%y %m') AND md.status='N' ";
		}
		...
		private void processingMessages(ArrayList<String> columndata, int currentRow) throws IOException {
			...
			if (messageparam.size() == columndata.size() - 1) {
				...
				for (int messageParamCount = 0, columnDataCount = 1; messageParamCount < messageparam.size() & 
					...
					parameterValue = columndata.get(columnDataCount);
			...
			if (messgeType == 'E' || messgeType == 'M') {
				...
				OrderNotificationData orderData = retrieveOrderData(messageTo, messgeType);
				if(null != orderData) {
					messageFrom = orderData.getOfficeEmail();
					footer = footer.replaceAll("<Reseller Name>", orderData.getOfficeName());
					footer = footer.replaceAll("<Contact Name>", orderData.getOfficeEmail());
					footer = footer.replaceAll("<Number>", orderData.getOfficePhoneNo());
				}
	...
	private static OrderNotificationData retrieveOrderData(String messageTo, char messgeType) {
		
		try {
			
			final String sql = "Select c.id from m_client c where c.email = ? or phone = ?";
			Long clientId = jdbcTemplate.queryForLong(sql, new Object[] {messageTo, messageTo});			
			return eventActionReadPlatformService.retrieveNotifyDetails(clientId, null);

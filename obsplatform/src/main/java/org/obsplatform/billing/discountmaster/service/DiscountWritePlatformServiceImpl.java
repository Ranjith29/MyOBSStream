...
public class DiscountWritePlatformServiceImpl implements DiscountWritePlatformService {
	...
	public CommandProcessingResult createNewDiscount(final JsonCommand command) {
		...
		try {
			...
			discountMaster=this.assembleDiscountDetails(discountPricesArray,discountMaster); 
	...
	private DiscountMaster assembleDiscountDetails(JsonArray discountPricesArray, DiscountMaster discountMaster) {
		  ...
		  String[]  discountPrices = new String[discountPricesArray.size()];
		...
		try {
			...
			final JsonArray discountPricesArray = command.arrayOfParameterNamed("discountPrices").getAsJsonArray();
			  String[]  discountPrices=new String[discountPricesArray.size()];
			    ...
			    Map<String, Object> pricesChanges=new HashMap<String,Object>();
				 for (String discountPrice : discountPrices) {
						...
						if(id != null){
						  ...
						  if(discountDetails != null){
							  Map<String, Object>  priceDetail=discountDetails.update(element,fromApiJsonHelper);
							   pricesChanges.putAll(priceDetail);
					 ...
					 discountMaster.getDiscountDetails().removeAll(details); /* Removing remaining Details like deleted details screen */	

			final Map<String, Object> changes = discountMaster.update(command);
			changes.putAll(pricesChanges);
	...
	public CommandProcessingResult deleteDiscount(final Long entityId) {
		...
		try {
			this.context.authenticatedUser();
			DiscountMaster	discountMaster = this.discountRetrieveById(entityId);

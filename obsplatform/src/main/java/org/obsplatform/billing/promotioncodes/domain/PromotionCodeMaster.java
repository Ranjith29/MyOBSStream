...
public class PromotionCodeMaster extends AbstractPersistable<Long> {
	...
	public static PromotionCodeMaster fromJson(final JsonCommand command) {
		...
		final String promotionDescription = command.stringValueOfParameterNamed("promotionDescription");
	...
	public Map<String, Object> updatePromotion(final JsonCommand command) {
		...
		if (command.isChangeInStringParameterNamed("promotionCode",this.promotionCode)) {
			final String newValue = command.stringValueOfParameterNamed("promotionCode");
			actualChanges.put("promotionCode", newValue);
			this.promotionCode = StringUtils.defaultIfEmpty(newValue, null);
		}

		if (command.isChangeInStringParameterNamed("promotionDescription",this.promotionDescription)) {
			final String newValue = command.stringValueOfParameterNamed("promotionDescription");
		...
		if (command.isChangeInStringParameterNamed("discountType",this.discountType)) {
			final String newValue = command.stringValueOfParameterNamed("discountType");
			actualChanges.put("discountType", newValue);
			this.discountType = StringUtils.defaultIfEmpty(newValue, null);
		}

		if (command.isChangeInBigDecimalParameterNamed("discountRate",this.discountRate)) {
			final BigDecimal newValue = command.bigDecimalValueOfParameterNamed("discountRate");
			actualChanges.put("discountRate", newValue);
			this.discountRate = newValue;
		}

		if (command.isChangeInLocalDateParameterNamed("startDate",new LocalDate(this.startDate))) {

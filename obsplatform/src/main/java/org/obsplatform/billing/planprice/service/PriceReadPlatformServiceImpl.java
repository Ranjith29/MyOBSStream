...
public class PriceReadPlatformServiceImpl implements PriceReadPlatformService {
	...
	public List<ServiceData> retrievePriceDetails(final Long planId,final String region) {
		...
		String sql = "SELECT p.plan_code AS plan_code,p.is_prepaid as isPrepaid,cp.id as contractId,pm.id AS id,pm.service_code AS serviceCode,"
				...
				+ "pr.priceregion_name AS priceregion,bcv.chargevariant_code AS variantCode FROM b_plan_master p,b_service se,b_charge_codes c,b_plan_pricing pm  left join b_priceregion_master "
				+ "pr on  pm.price_region_id=pr.id  LEFT JOIN b_contract_period cp ON cp.contract_period = pm.duration " +
				"  LEFT JOIN  b_chargevariant bcv ON pm.charging_variant=bcv.id WHERE p.id = pm.plan_id  AND pm.charge_code=c.charge_code and "
		...
		if (region != null) {
			...
			sql =" SELECT p.plan_code AS plan_code,p.is_prepaid AS isPrepaid,cp.id AS contractId,p.id as planId,pm.id AS id,pm.service_code AS serviceCode," +
			     ...
			     " pr.priceregion_name AS priceregion, bcv.chargevariant_code AS variantCode " +
			     ...
			     " LEFT JOIN b_chargevariant bcv ON pm.charging_variant=bcv.id " +
	...
	private static final class PriceMapper implements RowMapper<ServiceData> {
		...
		public ServiceData mapRow(final ResultSet rs, final int rowNum)	throws SQLException {
			...
			final Long chargingVariant = rs.getLong("chargingVariant");
			...
			final String variantCode = rs.getString("variantCode");
			...
			return new ServiceData(id, planCode, serviceCode, planDescription,chargeCode, chargingVariant, price, priceregion,
					contractId,duration, billingFrequency,discountId,isPrepaid,variantCode);
		}
	}
	...
	private static final class PricingMapper implements RowMapper<PricingData> {
		...
		public String schema() {
			...
			return " p.plan_id AS planId,pm.plan_code AS planCode,p.id AS priceId,p.service_code AS serviceCode,c.charge_code AS chargeCode,"
					+ " p.charging_variant AS chargeVariant,p.price AS price,p.discount_id AS discountId,p.duration as contractperiod," 
					+ " p.price_region_id AS priceregion,bcv.chargevariant_code AS variantCode "
					+ " FROM b_plan_pricing p,b_service s,b_charge_codes c,b_plan_master pm,b_chargevariant bcv WHERE p.charge_code = c.charge_code  AND (p.service_code = s.service_code or "
					+ " p.service_code ='None') AND pm.id = p.plan_id AND bcv.id=p.charging_variant ";
		...
		public PricingData mapRow(final ResultSet rs, final int rowNum) throws SQLException {
			...
			final String variantCode = rs.getString("variantCode");
			return new PricingData(planId, serviceCode, chargeCode, price,discountId, 
					    chargeVariant, priceregion, planCode,priceId, contractperiod,variantCode);
		}
	}
	...
	@Override
	public Long retrieveEventsPrice(Long planId) {
		try {
			this.context.authenticatedUser();
			String sql = "select sum(bmp.price) as price from b_plan_events bpe left join b_mod_master bmm on  bpe.event_name =bmm.id " +
							" left join b_mod_pricing bmp on bmp.event_id = bpe.event_name " +
							" where bpe.plan_id = "+planId+"  and bmm.status = 1 and bmp.is_deleted = 'n' and bpe.is_deleted = 'n'";

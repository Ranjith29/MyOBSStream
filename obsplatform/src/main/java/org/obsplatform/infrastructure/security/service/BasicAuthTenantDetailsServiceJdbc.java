...
/*    private static final class TenantMapper implements RowMapper<ObsPlatformTenant> {
        ...
        @Override
        public ObsPlatformTenant mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum) throws SQLException {
            final Long id = rs.getLong("id");
            final String tenantIdentifier = rs.getString("identifier");
            final String name = rs.getString("name");
            final String timezoneId = rs.getString("timezoneId");
            final ObsPlatformTenantConnection connection = getDBConnection(rs);
            return new ObsPlatformTenant(id, tenantIdentifier, name, timezoneId, connection);
        }

        // gets the DB connection
        private ObsPlatformTenantConnection getDBConnection(ResultSet rs) throws SQLException {
            ...
            return new ObsPlatformTenantConnection(connectionId, schemaName, schemaServer, schemaServerPort, schemaUsername,
    ...
    public ObsPlatformTenant loadTenantById(final String tenantIdentifier, final boolean isReport) {

...
public class JobDetailDataValidator {
	...
	public void validateForUpdateJobParameters(String json) {
        ...
        } else if (jobName.equalsIgnoreCase(SchedulerJobApiConstants.JOB_EVO_BATCH_PROCESSING)) {
        	
        	if (this.fromApiJsonHelper.parameterExists(SchedulerJobApiConstants.JOB_VALUE, element)) {
                atLeastOneParameterPassedForUpdate = true;
                final String value = this.fromApiJsonHelper.extractStringNamed(SchedulerJobApiConstants.JOB_VALUE, element);
                baseDataValidator.reset().parameter(SchedulerJobApiConstants.JOB_VALUE).value(value).notBlank();
            }
		}
